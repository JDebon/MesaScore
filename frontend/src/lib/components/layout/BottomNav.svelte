		...
		{#each tabs as tab}
			<a
				...
				class="flex flex-1 flex-col items-center gap-1 py-2 text-xs transition-colors
					{isActive(tab.href) ? 'text-primary-600' : 'text-text-secondary hover:text-text-primary'}"
			>
				{#if tab.icon === 'home'}
...
<!-- Desktop sidebar -->
<aside class="hidden lg:fixed lg:inset-y-14 lg:left-0 lg:flex lg:w-56 lg:flex-col lg:border-r lg:border-border lg:bg-surface">
